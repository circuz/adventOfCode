# -*- coding: utf-8 -*-
"""
Created on Wed Dec  8 12:19:24 2021

@author: August
"""

strings = ["dfgabce cadfgb cefa ca aecbg dfcegb geabd ecbfg cab agcfbe | egbfadc dbgae gcfeb abgdfc",
...
def decode(code):
    atog = ["a","b","c","d","e","f","g"]
    potentials = ["abcdefg"]*7
    numbers = code.split(" ")
    for letter in atog:
        if "".join(sorted(code)).strip().count(letter)==4:
            potentials[4] = letter
            potentials[3] = potentials[3].replace(potentials[4],"")
        if "".join(sorted(code)).strip().count(letter)==6:
            potentials[1] = letter
            potentials[3] = potentials[3].replace(potentials[1],"")
        if "".join(sorted(code)).strip().count(letter)==9:
            potentials[5] = letter
            potentials[3] = potentials[3].replace(potentials[5],"")
    for i in range(10):
        if(len(numbers[i])==3):
            potentials[0] = numbers[i].replace(potentials[5],"")
            potentials[2] = potentials[0]
            potentials[3] = potentials[3].replace(potentials[0][0],"")
            potentials[3] = potentials[3].replace(potentials[0][1],"")
    for i in range(10):
        if(len(numbers[i])==2):
            potentials[2] = numbers[i].replace(potentials[5],"")
            potentials[0] = potentials[0].replace(potentials[2],"")   
    potentials[6] = potentials[3]
    for i in range(10):
        if(len(numbers[i])==4):
            for c in numbers[i]:
                potentials[6] = potentials[6].replace(c,"")
    potentials[3] = potentials[3].replace(potentials[6],"")
    return(potentials)
...
def decipher(code,key):
    ...
    for i in range(4):
        if len(numbers[i]) == 2:
            ...
            if key[1] in numbers[i]:
                num += 5* 10**(3-i)
            elif key[4] in numbers[i]:
                num += 2* 10**(3-i)
            else:
                num += 3* 10**(3-i)
        elif len(numbers[i]) == 6:
            if key[3] not in numbers[i]:
                num += 0
            elif key[2] in numbers[i]:
                num += 9* 10**(3-i)
            else:
                num += 6* 10**(3-i)
    return(num)
